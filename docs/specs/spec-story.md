...
такие файлы только из доверенного источника. `tools/scene360-editor.html`
разбирает выбранный JS без выполнения, а запуск по пути доступен только в явно
включённом доверенном режиме. URL, абсолютные пути и выход через `..` в
редакторе запрещены.
