...
Обе формы регистронезависимы, скрывают кнопку статистики и записывают
эффективное значение `release` в сценарную переменную `mode`. Они не меняют
стартовую сцену и правила автосохранения. URL может повысить `debug` до
`release`, но не понижает заданный в `[meta]` режим `release`.
