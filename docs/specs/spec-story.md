...
JS-пакет `*-360.js` является исполняемым файлом, поэтому добавляйте в проект
только пакеты, созданные поставляемым конвертером или полученные из доверенного
источника. `tools/scene360-editor.html` разбирает импортированный `story360.js`
как JSON-данные и по умолчанию не выполняет найденные в нём пакеты. Для превью
выберите сам файл пакета в безопасном поле загрузки: редактор прочитает только
документированный `dataUrl` JPEG/PNG/WebP и JSON-метаданные. Доверенный
legacy-режим выполнения по пути нужен только для старого рабочего процесса,
включается явно и подтверждает каждый новый относительный путь. URL, абсолютные
пути и выход через `..` в редакторе запрещены. Эти ограничения относятся к
авторскому инструменту и не меняют формат или загрузку панорам готовой новеллой.
