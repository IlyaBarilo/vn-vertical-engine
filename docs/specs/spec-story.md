...
`360° PANORAMA PACKAGES` и обязаны ссылаться на `*-360.css`. По HTTP(S)
статистика выполняет только `HEAD`: ответы `404` и `410` означают отсутствие
пакета, успешный ответ подтверждает только наличие, а формат и размеры
проверяются рантаймом при открытии панорамы. Через `file://`, при блокировке
`HEAD` или неподдерживаемом ответе пакет получает статус `unverified`, а не
ложную ошибку отсутствия. Большое Base64-содержимое ради статистики не читается;
необязательный `*-360-mobile.css` отдельно не требуется.
