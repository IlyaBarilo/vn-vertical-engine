...
По умолчанию пункты меню выводятся с автоматической нумерацией. Чтобы отключить номера для конкретного меню, используйте `menu unnumbered` или `menu plain`. Для коротких вариантов можно включить компактную раскладку через `menu compact`: кнопки будут занимать ширину текста и переноситься на новую строку только когда не влезают. Компактная раскладка всегда скрывает номера, даже если вместе с ней указан `numbered`. Любая неизвестная опция после `menu` считается ошибкой.
