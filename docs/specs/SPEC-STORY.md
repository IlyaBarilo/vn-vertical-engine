...
По умолчанию пункты меню выводятся без автоматической нумерации. Чтобы включить номера для конкретного меню, используйте `menu numbered`; алиасы `menu numbers` и `menu number` работают так же. Для коротких вариантов можно включить компактную раскладку через `menu compact` или `menu fit`: кнопки будут занимать ширину текста и переноситься на новую строку только когда не влезают. Любая неизвестная опция после `menu` считается ошибкой.
