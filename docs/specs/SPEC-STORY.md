...
По умолчанию пункты меню выводятся без автоматической нумерации. Чтобы включить номера для конкретного меню, используйте `menu numbered`; алиасы `menu numbers` и `menu number` работают так же. Любая неизвестная опция после `menu` считается ошибкой.
