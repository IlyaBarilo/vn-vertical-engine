...
Авторские `story.js` и необязательный `story360.js` загружаются в отдельных
короткоживущих Web Worker. Основная страница получает только текст сценария и
проверенную JSON-подобную карту 360-пространств, поэтому формат файлов и ручное
редактирование не меняются, а их код не выполняется в DOM новеллы. Это
ограничение прав, а не замена JS пассивным форматом. Панорамные изображения
