...
`scene360-editor.html` импортирует `story360.js` как данные и не выполняет
указанные в нём JS-пакеты автоматически. Для безопасного предпросмотра выберите
нужный `*-360.js` в отдельном поле файла: редактор извлечёт растровое изображение
и метаданные без запуска JavaScript. Выполнение пакета по пути доступно только в
явно включаемом legacy-режиме для собственного или проверенного проекта и
требует подтверждения каждого нового пути. Внешние URL, абсолютные пути и `..`
редактор не принимает.
