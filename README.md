...
При управлении с клавиатуры Enter/Space продолжают диалог, Tab переводит фокус
между вариантами выбора, а служебные и игровые окна возвращают фокус после
закрытия. Просмотрщик фотографий из 360-меток закрывается клавишей Escape.
...
Автоматическая проверка разделена по уровням:

| Контур | Проверяемые браузеры |
| --- | --- |
| Обычный E2E в CI | Chromium и Firefox |
| Релизный ZIP на Windows | Microsoft Edge и Firefox через HTTP и настоящий `file://` |
| Android | Chrome и Firefox, ручная проверка на реальных устройствах |

Для Windows 10 и 11 рекомендуется актуальный Microsoft Edge. Windows 7, 8 и
8.1 не имеют поддерживаемого браузера Microsoft и не входят в поддерживаемые
окружения проекта. Edge Legacy, Internet Explorer и режим IE в Microsoft Edge
не поддерживаются; при открытии в Internet Explorer страница показывает
понятное сообщение с предложением использовать современный браузер.
...
> **При размещении новеллы на сайте рекомендуется отключить автосохранение
> в разделе `[meta]` файла `story.js`:**
>
> ```text
> [meta]
> autosave = false
> ```
>
> Если раздел `[meta]` уже есть, добавьте или измените в нём строку `autosave`.
> Эта настройка запрещает движку любые обращения к `localStorage`: чтение,
> создание, запись и удаление сохранений. Параметры URL, включая `nosave=false`,
> не могут отменить запрет автора. После обновления страницы или нажатия кнопки
> перезапуска прохождение начинается сначала; старые сохранения остаются нетронутыми.
> Для локального использования автосохранение можно оставить включённым.
>
> URL-параметр `nosave=true` позволяет запретить обращения к сохранениям
> для отдельного запуска без изменения файла сценария.
> `mode=release` сам по себе сохранения не отключает.

> Перед публикацией новеллы в интернете уточните применимые требования к сайту
> и обработке персональных данных. Необходимость политики, её содержание и способ
> размещения рекомендуется согласовать с юристом с учётом действующего
> законодательства и фактической работы сайта. Примеры настройки движка
> не являются образцами юридического оформления сайта.

## Интерфейс и режимы отображения

Кнопка ℹ️ «Информация» открывает сведения о движке. Через `aboutEnabled=true`
в `[meta]` можно добавить вкладку «О проекте», задать её заголовок `aboutTitle`
и строку `aboutText` с текстом, переносами `[br]` и ссылками. При наличии описания
окно открывается на проекте; иначе сразу показывает движок без вкладок. Формат
и пример с вымышленными данными приведены в
[спецификации сценария](docs/specs/spec-story.md#aboutenabled-abouttitle-abouttext).
...
HTML-игры всегда выполняются в строгом изолированном iframe. Значение
`engine.gameSandbox = strict` можно оставить явным для читаемости; режим
`legacy` и переопределение `sandbox` у отдельной игры больше не поддерживаются.
...
Авторские `story.js` и необязательный `story360.js` подключаются обычными
`<script>` для совместимого запуска через `file://` и обновления после F5.
Их код выполняется с правами основной страницы; перед запуском движка проверяются
текст сценария и JSON-подобная карта 360-пространств. Изолированный Worker-режим
сохранён в коде для возможного возврата, но по умолчанию не используется.
Панорамные изображения
загружаются только из декларативных `*-360.css`; пути `*-360.js` отклоняются.

Версии совместимости разделены и не записываются вместе в сценарий: номер
релиза принадлежит движку, текущую грамматику DSL определяет парсер,
`story360.js` содержит собственное поле `version: 1`, CSS-панорама — схему
`vn360-css-pack-v1`, а HTML мини-игры — meta `vn-game-protocol`. Поэтому обычное
обновление движка не требует менять `story.js`; версии файлов меняются только
при несовместимом изменении соответствующего формата.

Кнопка информации ℹ️ показывает версию движка. Текстовая статистика дополнительно
показывает текущие версии Story DSL, STORY360, Panorama CSS и протокола мини-игр,
а также `projectId` и фактический режим запуска `debug`/`release`.

Все авторские ресурсы сценария и `story360.js` загружаются только из каталога
`assets/`. Путь записывается относительно `index.html`, начинается с `assets/`,
использует прямые слеши `/` и не содержит URL, абсолютных компонентов, `..`,
query/hash или percent-encoding. Внутри `assets/` разрешена любая структура
подкаталогов. SVG допускается как изображение через `<img>` или CSS-фон, но не
встраивается как `iframe`, `object`, `embed` или inline-разметка; студенческий
аудитор дополнительно проверяет SVG на активное содержимое и чрезмерную сложность.
...
изоляции, ограничивает чувствительные браузерные API через Permissions Policy и
проверяет meta `vn-game-protocol=2`, возвращаемую версию и обязательные
`gameId`/`sessionId`. Движок допускает
только одну активную мини-игру одновременно.
Автономная студенческая игра хранит CSS и JavaScript внутри одного HTML и
обязательно задаёт строгий `meta Content-Security-Policy`, запрещающий сеть,
вложенные фреймы, Worker, формы и навигацию. Точный профиль CSP и готовый шаблон
...
| --- | --- |
| `student-project-auditor.html` | Проверяет весь каталог студенческой новеллы по allowlist и готовит пакет для проверки в ИИ-чате |
| `scene360-editor.html` | Собирает `story360.js`, связывает панорамы и расставляет точки перехода |
| `convert-360-img-to-css.html` | Упаковывает панорамные изображения в автономные CSS-пакеты и переносит данные из старых JS-пакетов без их выполнения |
...
Перед размещением студенческой работы аудитор отклоняет любой файл вне
разрешённого состава, включая серверные скрипты, конфигурации, dotfiles,
манифесты и двойные расширения. На VPS допущенные файлы следует переносить в
новый каталог, обслуживаемый только как статический. Подробный порядок приведён
в [инструкции по проверке студенческих проектов](docs/student-project-review.md).
...
старых пакетов `*-360.js` в редакторе отключены. Пути из импортированной карты не
выполняются автоматически, а движок полностью отвергает JS-пакеты изображений.
Внешние URL, абсолютные пути и `..` редактор не принимает.
...
<p align="center">
  <img src="docs/tools/converter-img360-to-js.webp" width="300" alt="Конвертер панорамного изображения в автономный CSS-пакет">
...
Mermaid в строгом режиме; пользовательские подписи экранируются, а готовый SVG
дополнительно очищается перед добавлением в страницу.
...
При открытии полного графа или графа ресурсов отдельная полоса показывает
подготовку Mermaid и загрузку всех выводимых обычных изображений, обложек,
персонажей и CSS-панорам. После завершения она остаётся видимой как итоговый
маркер со счётчиком ошибок и временем; сама схема параллельной загрузки графа
при этом не ограничивается и не меняется.
...
│   ├── story-sandbox-loader.js         загрузка авторских JS и сохранённый Worker-режим
...
│   ├── student-project-auditor.html   аудитор студенческой новеллы и мини-игр
...
Оба релизных архива содержат `release-manifest.json`: в нём указаны версия
движка, исходный Git-коммит, версии поддерживаемых форматов и SHA-256 файлов
runtime. Это позволяет точно определить состав распакованной версии.
