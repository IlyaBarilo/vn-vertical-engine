...
│   ├── TESTING.md                     запуск автотестов движка
...
│
├── tests/                              регрессионные тесты движка
├── package.json                        необязательные npm-команды для тестов
├── .github/workflows/
...
- [Автоматические тесты движка](docs/TESTING.md)
...
набор доступен Codex и локально без установки npm-пакетов. Браузерные проверки
реального интерфейса, `localStorage` и iframe запускаются отдельно командой
`npm run test:e2e` после `npm ci` и установки Chromium. GitHub Actions выполняет
оба набора. Автотесты проверяют код движка и не заменяют статистику и графы
конкретной новеллы. Подробнее: [docs/TESTING.md](docs/TESTING.md).
