...
После изменения парсера или логики проверок выполните `node --test`. Эта же
команда запускается в GitHub Actions и доступна Codex без установки npm-пакетов.
Автотесты проверяют код движка и не заменяют статистику и графы конкретной
новеллы. Подробнее: [docs/TESTING.md](docs/TESTING.md).
