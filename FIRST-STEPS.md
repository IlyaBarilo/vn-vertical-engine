...
`360° PANORAMA PACKAGES`: по HTTP движок выполняет только лёгкий `HEAD`-запрос,
а через `file://` не читает большие Base64-пакеты ради статистики и показывает
статус «не проверено» до фактического открытия панорамы. Отсутствующий
`*-360.css` поэтому не смешивается со списком обычных `MISSING FILES`.
