...
Оставьте `engine.gameSandbox = strict` в новой новелле как явное подтверждение
изоляции HTML-мини-игр. Runtime всегда использует строгий режим; `legacy` и
переопределение `sandbox` у отдельной игры больше не поддерживаются. Старую игру,
несовместимую со строгим режимом, необходимо перевести на протокол версии 2.
...
- `tools/convert-360-img-to-css.html` — преобразовать панорамы в автономные CSS-пакеты или безопасно перенести данные из старых JS-пакетов без их выполнения;
...
- содержит в `head` маркер `vn-game-protocol=2`;
- возвращает `protocolVersion`, полученные `gameId` и `sessionId` в финальном `gameResult`;
- после завершения не продолжает принимать ввод.

Тестер всегда запускает игру в строгом sandbox и не предоставляет режима
`Legacy`. Если старая игра не проходит проверку, приведите её к протоколу v2 и
устраните зависимости от дополнительных прав iframe.
...
CSS-панорам, сценария, условий story360, достижимости сцен и циклов. Красный
крестик указывает на подробный раздел ниже, жёлтое предупреждение — на ресурс,
который безопасно проверяется только при фактическом открытии.
...
Обычные изображения проверяются загрузкой и отдельно сверяются с требованиями
к размерам. Декларативные 360-изображения вынесены в раздел
`360° PANORAMA PACKAGES`. После первого открытия статистики движок в фоне по
полностью загружает выбранные normal/mobile `*-360.css`: одновременно до четырёх
пакетов на компьютере и до двух на телефоне. Для каждого проверяются схема,
Base64, MIME, сигнатура, размеры и декодирование изображения. Полоса показывает
число готовых пакетов. Проверка продолжается после закрытия окна, а при
завершении открытая текстовая статистика обновляется один раз. В памяти до F5
остаются только статусы и небольшие метаданные; большие Blob и изображения
освобождаются после каждого пакета. Отсутствующий `*-360.css` не смешивается со
списком обычных `MISSING FILES`.
...
`0`, `no` или `off` явно отключают URL-флаг, но не отменяют `autosave = false`
в `[meta]`.
...
При включённом автосохранении кнопка перезапуска очищает только активный слот
при обычном запуске или в режиме `novel`. В режимах `scene`, `nosave` и при
`autosave = false` в `[meta]` она заново запускает точку входа, не удаляя
сохранения. Неизвестное или неоднозначное по регистру имя сцены показывает ошибку.
Настройка `autosave = false` запрещает любые обращения движка к `localStorage`,
включая чтение, создание, запись, миграцию и удаление сохранений. Параметры URL
не могут отменить этот запрет. Режимы `scene` и `nosave` также отключают storage.

---

### Сведения о сайте или новелле

Добавьте в существующий `[meta]` настройки вкладки «О проекте»:
...
Кнопка ℹ️ «Информация» доступна с начала новеллы. Если авторское описание
включено, окно открывается на вкладке «О проекте»; вкладка «О движке» показывает
версию программы, лицензию и разработчика. `aboutTitle` выводится над авторским
текстом и не меняет короткие названия вкладок. Вся строка `aboutText`
записывается на одной строке файла: `[br]` переносит текст, `[br][br]` разделяет
абзацы. Эти маркеры не добавляют форматирование диалогам.
В примере указаны вымышленные данные и адрес документа; замените их перед
публикацией. Веб-ссылки открываются по нажатию в новой вкладке.

Если авторский раздел не нужен, задайте `aboutEnabled=false` или удалите его
настройки. Кнопка ℹ️ останется и будет показывать движок без вкладок.
